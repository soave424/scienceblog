...
export default function WritePage() {
  ...
  const [chatMessages, setChatMessages] = useState<{ role: 'ai' | 'user', text: string }[]>([
    { role: 'ai', text: "안녕하세요! 탐험 대원님. 오늘 관찰한 사진을 올리거나 내용을 적어보세요. 제가 더 멋진 관찰을 할 수 있도록 도와드릴게요!" }
  ]);
  ...
  const triggerVisionGuidance = async (imageUrl: string) => {
    ...
    setChatMessages(prev => [...prev, { role: 'ai', text: "탐험 대원님이 올린 사진을 돋보기로 자세히 살펴보고 있어요... 잠시만요!" }]);
    ...
    try {
      ...
      if (result.questions && result.questions.length > 0) {
        ...
        setChatMessages(prev => [...prev, { role: 'ai', text: "사진을 보니 이런 것들이 궁금해졌어요! 관찰 일지에 답을 적어볼까요?" }]);
      ...
      toast({
        variant: "destructive",
        title: "이미지 분석 오류",
        description: "AI가 사진을 분석하는 중에 문제가 생겼어요. 다시 시도해볼까요?",
      });
    } finally {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col" suppressHydrationWarning>
      ...
      <main className="flex-1 flex overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto space-y-6">
            <div className="flex items-center justify-between">
              <div className="space-y-1 flex-1 mr-4">
                ...
                <div className="flex items-center gap-4 text-sm text-muted-foreground font-bold">
                  <span className="bg-primary/10 px-3 py-1 rounded-full text-primary">탐험 대원 기록</span>
                  <span>|</span>
                  <span>오늘의 날씨: 맑음 ☀️</span>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <Button variant="outline" className="rounded-2xl border-2 font-bold hover:bg-primary/5">
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {images.map((img, idx) => (
                  <div key={idx} className="relative aspect-video rounded-3xl overflow-hidden border-4 border-white bg-muted group shadow-xl hover:scale-105 transition-transform">
                ...
                >
                  ...
                  <span className="text-sm font-black">{isImageAnalyzing ? "분석 중..." : "관찰 사진 올리기"}</span>
                </button>
              </div>
        ...
        {/* AI Sidebar */}
        <aside className="w-[400px] border-l-4 border-primary/10 bg-white flex flex-col shadow-2xl z-10" suppressHydrationWarning>
          <div className="p-8 border-b bg-primary/5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="font-headline font-black text-2xl text-primary leading-none mb-1">AI 탐험 코치</h2>
                <p className="text-xs text-muted-foreground font-bold uppercase tracking-widest">Real-time Coaching</p>
              </div>
            </div>
          </div>
              ...
              <Input 
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
                    ...
                    setTimeout(() => {
                      setChatMessages(prev => [...prev, { role: 'ai', text: "좋은 질문이에요! 그 부분은 관찰 일지에 숫자로 더 자세히 기록하면 완벽할 것 같아요." }]);
                    }, 1000);
                  }
                }}
              />
              <Button size="icon" className="absolute right-3 top-1/2 -translate-y-1/2 h-12 w-12 bg-primary hover:bg-primary/90 rounded-2xl shadow-lg">
